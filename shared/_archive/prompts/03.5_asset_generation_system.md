...
```bash
uv run assets                    # Uses current reel, provider=openai by default
uv run assets -p gemini          # Use Gemini instead
...
```
❌ Error: 03_visual_plan.output.json not found
...
```
❌ Error: Invalid JSON in 03_visual_plan.output.json
...
```
⚠️ Warning: Failed to generate {asset_id}
...
- **Stage 3:** Visual Plan (creates prompts) → outputs `03_visual_plan.output.json`
...
4. For each asset:
   ...
   - Call: `python scripts/generate_asset.py --prompt "..." --output "..." --provider openai`
